...
standalone .tex report and depends on ``random_augmented.json``, which is
NOT committed (it is an extra aggregate emitted by a fresh
``run_random_eval.py`` run into its /tmp OUTDIR). The paper's Table V
Hops/PLU numbers are reproducible WITHOUT this script: see
``regen_density_figs.py``, which computes the same means/CIs directly from
the committed ``dataset/random_eval_results.json``. Run this only if you
re-ran ``run_random_eval.py`` and want the full report.
