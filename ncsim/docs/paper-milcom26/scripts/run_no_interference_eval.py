...
def run_averaged(yaml_str, base_label, routing, greedy_order, scheduler):
    """Run NUM_SEEDS replications; return {makespan_mean, ms[]} or None."""
    def _run(seed):
    ...
    if not ms_list:
        return None
    return {"makespan_mean": sum(ms_list) / len(ms_list), "ms": ms_list}
...
def main():
    ...
    rand_perseed  = {}   # (dlabel, dag_label, sched, rlabel) → {ms[]}
        ...
        for dag_label, dag_fn in RAND_DAGS:
            ...
            for sched in SCHEDULERS:
                for rlabel, routing, greedy_order in RAND_STRATEGIES:
                    ...
                    if agg:
                        rand_perseed[(dlabel, dag_label, sched, rlabel)] = {"ms": agg["ms"]}
                    print(f"{ms:.3f}s" if ms is not None else "FAILED")
    ...
    with open(json_path, "w") as f:
        json.dump({
            ...
            "rand_perseed": {f"{d}|{g}|{s}|{r}": v
                             for (d, g, s, r), v in rand_perseed.items()},
