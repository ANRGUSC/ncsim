...
\newcommand{\heftone}{HEFT\nobreakdash-1}
\newcommand{\hefttwo}{HEFT\nobreakdash-2}
...
DAG scheduling on heterogeneous platforms. The SAGA
framework~\cite{coleman2024saga} provides reference implementations for
benchmarking. These works adopt a static pairwise-bandwidth model that does
not capture interference.
...
remains the standard analytical baseline. Interference-aware routing in ad
hoc networks is a long-standing topic, but the schedulers it supplies
typically optimize a single point-to-point flow rather than the
joint placement-and-routing problem of a DAG workload.

To our knowledge, the systematic evaluation we present here---HEFT
bandwidth-matrix variants combined with an inventory of routing schemes,
under a realistic CSMA/CA interference model and across a range of
topologies and DAG sizes---has not previously appeared in the literature.

%======================================================================
\section{Conclusion}
\label{sec:conclusion}

Wireless interference inverts the intuition that interference-oblivious
HEFT scheduling rests on: a placement that looks well load-balanced under a
widest-path bandwidth model produces simultaneously active links whose
mutual interference dominates the makespan. A simple modification---a
heavy bandwidth penalty on non-adjacent task pairs (\heftone{})---restores
performance by encouraging single-hop placement. Among nine routing
schemes, the simplest interference-unaware shortest-by-hop rule wins
broadly; deferred dynamic routing (GSD-D) provides additional gains where
sparse topologies leave no good alternative paths and the only useful lever
is timing.

Future work will extend the evaluation to mobility (where the topology
itself changes during execution) and to multi-DAG workloads with online
