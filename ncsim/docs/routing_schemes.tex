...
\begin{itemize}
\item Many transfers compete for the same few links
\item Waiting 1--2 transfer completions frees enough airtime for a much
      faster path
\item The DAG has enough parallelism that deferring one transfer does not
      immediately stall the critical path
\end{itemize}

On sparse grids or small DAGs, contention is mild enough that deferral adds
latency without benefit---Shortest-Path (S) is typically sufficient.
