...
### Lua Architecture: Data vs Execution (2026-01-24)

**Problem:** Running `runconfig` (the config GUI) would overwrite SAGA scheduler integration code.

**Root Cause:** The config generator was outputting BOTH data AND execution logic into `iobt-config.lua`. The execution logic (ConfiguredDagState, InitConfiguredDag, etc.) would overwrite the SAGA-aware version.

**Solution - Separation of Concerns:**
```
iobt-config.lua (generated by runconfig)
├── IoBTConfig table with:
│   ├── dagTasks = [{id, name, deps}, ...]  -- DAG structure
│   ├── dagSettings = {taskDuration, transferDuration}
│   ├── units = [...]  -- Infantry/vehicle definitions
│   ├── buildings = [...]  -- Base structures
│   └── networkSettings = {...}
└── Helper functions for unit generation

iobt-main.lua (permanent, never overwritten)
├── ConfiguredDagState  -- Execution state
├── InitConfiguredDag()  -- SAGA-aware initialization
├── RequestSagaSchedule()  -- Sends DAG to SAGA service
├── ApplySagaAssignments()  -- Applies HEFT assignments
├── ExecuteNextDagTasks()  -- Task execution
├── CompleteTask()  -- Task completion
├── WorldLoaded()  -- Entry point
└── Tick()  -- Per-frame updates
```

**Key Principles:**
1. **Data in config** - `iobt-config.lua` defines WHAT (tasks, units, settings)
2. **Logic in main** - `iobt-main.lua` defines HOW (SAGA integration, execution)
3. **Config reads data** - `IoBTConfig.dagTasks` is read by execution logic
4. **Regeneration safe** - Running `runconfig` won't break SAGA integration

**Load Order (in rules.yaml):**
```yaml
LuaScript:
    Scripts: iobt-config.lua, iobt-main.lua
```
Config loads first (defines data), then main loads (defines execution logic that reads the data).

### JSON Number Parsing in C# (2026-01-24)

**Problem:** SAGA scheduler assignments not being stored - all assignments showed as 0.

**Root Cause:** System.Text.Json returns all numbers as `System.Double`, not `long` or `int`.

**Error Output:**
```
[BRIDGE] Assignment T0: value type = System.Double, value = 2
[BRIDGE] SKIPPED: T0 - value type not recognized
```

**Fix:** Handle `double` type in addition to `long` and `int`:
```csharp
...
else if (kvp.Value is double nodeIndexDouble)  // THIS WAS MISSING
{
    pendingAssignments[taskId] = (int)nodeIndexDouble;
}
```

**Lesson:** Always include `double` type handling when parsing JSON numbers in C# with System.Text.Json.
