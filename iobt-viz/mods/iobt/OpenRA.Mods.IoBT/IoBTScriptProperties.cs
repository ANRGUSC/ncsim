...
namespace OpenRA.Mods.IoBT
{
	...
	public class IoBTGlobalScriptProperties : ScriptPlayerProperties
	{
		...
		[Desc("Add a task-to-task data transfer. Shows as 'T1→T4' in the status panel and highlights the link.")]
		public void AddTaskTransfer(string sourceTaskId, string destTaskId, string label)
		{
			var overlay = GetOverlay();
			overlay?.AddTaskTransfer(sourceTaskId, destTaskId, label);
		}

		[Desc("Add a task-to-task data transfer with explicit node indices. Use when destination task isn't assigned yet.")]
		public void AddTaskTransferWithNodes(string sourceTaskId, string destTaskId, int fromNodeIndex, int toNodeIndex, string label)
		{
			Log.Write("debug", $"IoBT Lua: AddTaskTransferWithNodes called from Lua: {sourceTaskId}->{destTaskId}");
			var overlay = GetOverlay();
			if (overlay == null)
				Log.Write("debug", "IoBT Lua: overlay is NULL!");
			overlay?.AddTaskTransferWithNodes(sourceTaskId, destTaskId, fromNodeIndex, toNodeIndex, label);
		}
