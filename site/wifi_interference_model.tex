...
\item \textbf{Hidden terminal interference}: Links outside each other's
  carrier sensing range may transmit simultaneously, causing frame loss
  at the receiver (capture-threshold model).
\end{enumerate}
...
\item \textbf{Hidden terminals} (active links \emph{not} in the conflict graph):
  These may transmit simultaneously, causing frame loss at the receiver
  if SINR falls below the capture threshold.
\end{enumerate}
...
\paragraph{Hidden terminal factor (capture threshold).}
If $\mathcal{H}_\ell \neq \emptyset$, the SINR at the receiver of $\ell$ is
computed per \Cref{eq:sinr}, with interference from hidden terminal
transmitters only.  In 802.11, each frame is transmitted at a pre-selected
MCS and either decoded successfully or lost entirely---there is no fallback
to a lower MCS during reception.  The decode threshold for a given MCS is
lower than the rate-selection threshold by a capture margin $\Delta = 5$\,dB:

\begin{equation}
\theta_\text{decode} = \theta_\text{select} - \Delta
\end{equation}

The hidden terminal factor is then:
\begin{equation}
...
If no hidden terminals are active, $f_\text{HT} = 1.0$.
...
\boxed{f_\text{bianchi}(\ell) = f_\text{HT}(\ell) \cdot f_\text{contention}(\ell)
= f_\text{HT}(\ell) \cdot \frac{\eta(n)}{n}}
\end{equation}
...
When a transfer starts or completes on a link, all other active links are
recalculated (both conflict graph neighbors for contention and hidden
terminals for SINR):

\begin{equation}
\texttt{get\_affected\_links}(\ell) = \mathcal{A} \setminus \{\ell\}
\end{equation}
...
Interference factor  & $1.0$ (static) & $f_\text{HT} \cdot \frac{\eta(n)}{n}$ (dynamic) \\
Contention model     & Worst-case clique bound & Bianchi MAC + active count \\
Hidden terminals     & Not modeled & Capture-threshold model \\
...
f_\text{HT} &= 1.0 \text{ (no hidden terminals---both links are in conflict graph)} \\
...
\item \textbf{Two complementary interference variants}: a fast static clique
  ...
  and hidden terminal capture-threshold effects.
