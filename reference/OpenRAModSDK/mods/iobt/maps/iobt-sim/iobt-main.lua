...
function StartDagDemo()
    ...
    -- Phase 2: T0 completes, data transfer to C1, start T1
    Trigger.AfterDelay(100, function()
        print("IoBT DAG: T0 complete, transferring data to C1")
        Blue.CompleteTask("T0")
        Blue.AddActiveLink(0, 1, "data")
    end)

    Trigger.AfterDelay(125, function()
        print("IoBT DAG: Starting T1 on C1")
        Blue.ClearActiveLinks()
        Blue.AssignTask("T1", 1)
    end)

    -- Phase 3: T1 completes, data transfer to C2, start T2
    Trigger.AfterDelay(200, function()
        print("IoBT DAG: T1 complete, transferring data to C2")
        Blue.CompleteTask("T1")
        Blue.AddActiveLink(1, 2, "result")
    end)

    Trigger.AfterDelay(225, function()
        print("IoBT DAG: Starting T2 on C2")
        Blue.ClearActiveLinks()
        Blue.AssignTask("T2", 2)
    end)
